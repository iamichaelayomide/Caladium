...
export function LogoTicker() {
  ...
  return (
    <section className="border-y border-border bg-transparent py-12">
        ...
        <div className="group overflow-hidden">
          <div className="marquee-track flex animate-marquee gap-12 group-hover:[animation-play-state:paused]">
            {duplicated.map((logo, index) => (
              <div key={`${logo}-${index}`} className="min-w-[160px] text-center text-lg font-semibold tracking-tight text-white/35">
                {logo}
              </div>
            ))}
          </div>
